...
class TestProducts(APITestCase):
    ...
    def test_get_future_goals(self):
        webClient = self.client
        response = webClient.get('/getFutureGoals/1', )
        self.assertEqual(response.status_code, 200)

    def test_get_past_goals(self):
        webClient = self.client
        response = webClient.get('/getPastGoals/1', )
        self.assertEqual(response.status_code, 200)

    def test_modify_goal(self):
        webClient = self.client
        response = webClient.put(
            '/updateGoalData/5', {'categoryIncomeGoal': 4000, 'categoryId': 1}, format="json")
        self.assertEqual(response.status_code, 200)
        goal = g.getAllGoals().filter(goal_id=5).first()
        category = c.getAllCategories().filter(category_id=1)
        categoryBis = category.first()
        goal_category_to_change = Goal_Category.getAllGoalCategories().filter(
            category=categoryBis).filter(goal=goal).first()
