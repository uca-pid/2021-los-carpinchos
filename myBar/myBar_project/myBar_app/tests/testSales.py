...
class TestProducts(APITestCase):
    ...
    def test_sale_creation_endPoint(self):
        ...
        response = webClient.post(
            '/createSale/1', {'creation_date': "13/10/21 19:26:48", 'products': [{'productId': 1, 'amount': 1}, {'productId': 2, 'amount': 1}]}, format='json')
    ...
    def test_modify_sale_details(self):
        ...
        response = webClient.put(
            '/updateSaleData/1', {"creation_date": "29/10/21 21:00:00", 'amount': 3, 'productId': 1}, format="json")
        self.assertEqual(response.status_code, 200)
        sale = s.getAllSales().filter(sale_id=1).first()
        self.assertEqual(sale.creation_date, datetime.date(2021, 10, 29))
    ...
    def test_get_sales_by_date(self):
        webClient = self.client
        response = webClient.post('/getAllSalesByDate/1',
                                  {'from': "10/03/21", 'to': "10/10/21"}, format="json")
        self.assertEqual(len(response.data), 8)

    def test_get_income_by_category(self):
        webClient = self.client
        response = webClient.post('/getIncomeByCategory/1',
                                  {'from': "01/10/21", 'to': "31/10/21"}, format="json")
